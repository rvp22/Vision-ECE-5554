...
rows = size(im,1);
cols = size(im,2);
% image_initial=im(1:rows-mod(rows,2), 1:cols-mod(cols,2), :);
image_initial=im;
...
%         image_reduced_size(1)=(image_initial_size(1)/2);  % Reduction Factor is 2
%         image_reduced_size(2)=(image_initial_size(2)/2);
%         
...
%             end
         ...
         % image_downsampled = imresize(image_filtered,0.5,'nearest');
          ...
          %image_upsampled = imresize(G{i},2,'nearest');
           image_upsampled = imresize(G{i},size(cell2mat(G(i-1))),'nearest');
